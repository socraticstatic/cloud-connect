...
describe('AI Fabric · Cost', () => {
  ...
  /* The failure this guards is a screen stating a saving while every row on
     it reads zero. The summary sentence must match the state the engine is
     actually in. */
  it('never claims a saving the meters do not support', () => {
    at(<AiCostPage />);
    const totals = aiSpendTotals(CC);

    if (totals.tokensToday === 0) {
  ...
  /* Govern lists EVERY token policy; only metered app tags carry a meter, so
     ...
     wrong. The screen must account for the difference, and account for it
     from the engine — not from a sentence someone remembered to write. */
  it('accounts for every token policy Govern lists, metered or not', () => {
    ...
    // The two counts together cover Govern's table exactly.
  ...
  /* Cost is stated on two screens. They must be the same figure — not
     "close", the same string — or one of them is lying. */
  it('states the same spend the Observe screen states', () => {
    ...
    at(<AiCostPage />);
    expect(
      within(tile('Spend today')).getByText(kpis.find(k => k.key === 'cost')!.value),
    ).toBeInTheDocument();
  });
});
