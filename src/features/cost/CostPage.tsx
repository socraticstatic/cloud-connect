    ...
    <main className="mx-auto max-w-6xl p-6">
      ...
      <div data-tour="cost-hero" className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatTile label="Savings identified" value={`$${identified.toLocaleString()}/mo`}
                  delta={{ text: `${billing.forecast} forecast`,
                           tone: egress.pub > PUBLIC_EXPOSURE_ALERT_USD ? 'neutral' : 'good',
                           // A rising (+) cost forecast is caution, not error: TrendingUp
                           // marks it as attention without amber and without red.
                           icon: billing.forecast.startsWith('+') ? TrendingUp : undefined }} />
        ...
        <StatTile label="Public exposure" value={`$${billing.uncommitted.toLocaleString()}/mo`}
                  delta={{ text: billing.uncommitted > 0 ? 'uncommitted, no SLA' : 'fully committed',
                           tone: billing.uncommitted > 0 ? 'neutral' : 'good',
                           // Globe = internet-exposed spend; reads as attention vs plain neutral.
                           icon: billing.uncommitted > 0 ? Globe : undefined }} />
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="space-y-6">
          ...
          <section aria-labelledby="trend-h" className="rounded-xl border border-slate-200 bg-white p-4">
            <h2 id="trend-h" className="mb-3 text-sm font-semibold text-slate-900">Daily egress spend, trailing 60 days</h2>
